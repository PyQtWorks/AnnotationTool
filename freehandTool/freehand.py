...
class FreehandTool(TurnGeneratorMixin, LineGeneratorMixin, CurveGeneratorMixin, QObject):
    ...
    def _initFilterPipe(self, startPosition):
        ...
        They feed to each other in same order of creation.
         '''
        self.turnGenerator = self.TurnGenerator(
            startPosition)  # call to generator function returns a generator
        # Execute preamble of generator and pause at first yield
        self.turnGenerator.send(None)
        self.lineGenerator = self.LineGenerator(startPosition)
        self.lineGenerator.send(None)
        self.curveGenerator = self.CurveGenerator(
            PathLine.nullPathLine(startPosition))
  ...
  """
    ...
    def pointerPressEvent(self, pointerEvent):
        ...
        # if pointerEvent.button() == Qt.LeftButton:
        #     return
    ...
    def pointerReleaseEvent(self, pointerEvent):
        ...
        # Hide.  Client knows about it but shouldn't be concerned with hiding, and may be reusing it.
  ...
  """
    ...
    """
        result = self.view.mapToScene(
            pointVCS)  # self knows it's view which maps
